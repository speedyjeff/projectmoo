    ...
    public class MooStartMenu : Menu
    {
        ...
        public override void Draw(IGraphics g)
        {
            ...
            g.DisableTranslation();
            {
                ...
                top += 25;
                g.Text(RGBA.Black, left + 250 + 140, top, string.Format("[5] bow ({0} wood, {1} rock, {2} level)", MooBow.WoodCraftCost, MooBow.RockCraftCost, MooBow.LevelCraftCost), 8);
                top += 15;
                g.Text(RGBA.Black, left + 250 + 140, top, string.Format("    or arrows ({0} wood)", MooBow.WoodCraftCost), 8);
                top += 15;
                g.Text(RGBA.Black, left + 250 + 140, top, string.Format("[6] Sword ({0} wood, {1} rock, {2} level)", MooSword.WoodCraftCost, MooSword.RockCraftCost, MooSword.LevelCraftCost), 8);
                top += 15;
                g.Text(RGBA.Black, left + 250 + 140, top, string.Format("[7] Turret ({0} wood, {1} rock, {2}, food, {3} level)", MooTurret.WoodCraftCost, MooTurret.RockCraftCost, MooTurret.FoodCraftCost, MooTurret.LevelCraftCost), 8);
                top += 15;
                //g.Text(RGBA.Black, left + 250 + 140, top, string.Format("[8] Landmine ({0} rock, {1}, food, {2} level)", MooTurret.RockCraftCost, MooTurret.FoodCraftCost, MooTurret.LevelCraftCost), 8);
                top += 15;
                g.Text(new RGBA() { R = 0, G = 127, B = 64, A = 255 }, left + 250 + 140, top, "[z] Zombies", 8);
                top += 40;
